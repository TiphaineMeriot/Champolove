    ...
    public void creation_Profil(Modele mod) throws Exception {
        ...
        if (rdmgenre<=8){
            if (Objects.equals(p.genre,"HOMME")){
                p.attirance.add("FEMME");
            }
            else if (Objects.equals(p.genre,"FEMME")){
                p.attirance.add("HOMME");
            }
            else{
                p.attirance.add("HOMME");
                p.attirance.add("FEMME");
            ...
            p.attirance.add(p.genre);
        }
        else{
            p.attirance.add("HOMME");
            p.attirance.add("FEMME");
            p.attirance.add("AUTRE");
        }
        // 2) age
        ...
        if(rdmage==0){
            ...
            p.agemin=entierAlea(18,50);
            p.agemax=entierAlea(30,80);
        }
        // 3) qualités : on rire un random entre 0 et 2 pour le nombre de qualités exigées puis on va chercher dans la liste des qualités 2 qualité au hasard
        // et on le stocke dans p.choix_qualites
        int nbqualites=entierAlea(0,2);
        for (int i=0;i<nbqualites;i++){
            int rdmqualite=entierAlea(0,mod.qualite.size()-1);
            p.choix_qualite.add(mod.qualite.get(rdmqualite));
        }
